import echartsYunNanMapOption from './EchartsYunNanMapOption.js';

import pieInProfessionPercentage from './PieInProfessionPercentage.js';
//数据转换
/**
 * 数据结构说明
	...
	dataInit: function (data, option) {
		...
		for (let key in data) {
			...
			if (key === 'x') {
				...
				let maxLeft = -100000000;
				let minRight = 100000000;
				let maxRight = -100000000;
				...
				for (let childKey in oneData) {
					...
					if (keyValue.indexOf("%") < 0 && keyValue.indexOf('居民消费价格指数') < 0 &&
						keyValue.indexOf('工业生产者出厂价格指数') < 0) {
						init.yAxisIndex = 0;
						init.type = 'bar';
				...
				if (minLeft !== 100000000) {
					yData.yAxis[0] = {
						min: minLeft,
						max: maxLeft,
					}
				}
				if (minRight !== 100000000) {
					yData.yAxis[1] = {
						min: minRight,
						max: maxRight,
					}
